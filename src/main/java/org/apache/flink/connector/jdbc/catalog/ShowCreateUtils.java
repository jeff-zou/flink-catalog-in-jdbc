...
public class ShowCreateUtils {
  ...
  static String buildCreateFormattedPrefix(
      String tableType, String tableName, boolean isTemporary) {
    return String.format(
        "CREATE %s%s %s (%s",
        isTemporary ? "TEMPORARY " : "", tableType, tableName, System.lineSeparator());
  }
  ...
  static String extractFormattedColumns(ResolvedCatalogBaseTable<?> table, String printIndent) {
    return (String)
        table.getResolvedSchema().getColumns().stream()
            .map(
                (column) -> {
                  return String.format("%s%s", printIndent, getColumnString(column));
                })
                ...
                .map(
                    (watermarkSpec) -> {
                      return String.format(
                          ...
                          printIndent,
                          EncodingUtils.escapeIdentifier(watermarkSpec.getRowtimeAttribute()),
                ...
                .map(
                    (entry) -> {
                      return String.format(
                          "%s'%s' = '%s'",
                          printIndent,
                          EncodingUtils.escapeSingleQuotes((String) entry.getKey()),
                          EncodingUtils.escapeSingleQuotes((String) entry.getValue()));
                    })
                .collect(Collectors.joining(",\n")))
        : Optional.empty();
  }
  ...
  static String buildShowCreateTableRow(
      ResolvedCatalogBaseTable<?> table, String tableName, boolean isTemporary) {
    if (table.getTableKind() == CatalogBaseTable.TableKind.VIEW) {
      ...
      String printIndent = "  ";
      StringBuilder sb =
          (new StringBuilder()).append(buildCreateFormattedPrefix("TABLE", tableName, isTemporary));
      ...
      extractFormattedComment(table)
          .ifPresent(
              (c) -> {
                sb.append(String.format("COMMENT '%s'%s", c, System.lineSeparator()));
              });
      extractFormattedPartitionedInfo((ResolvedCatalogTable) table)
          .ifPresent(
              (partitionedInfoFormatted) -> {
                sb.append("PARTITIONED BY (").append(partitionedInfoFormatted).append(")\n");
              });
  ...
  static String buildShowCreateViewRow(
      ResolvedCatalogBaseTable<?> view, String tableName, boolean isTemporary) {
    if (view.getTableKind() != CatalogBaseTable.TableKind.VIEW) {
      ...
      if (view.getOrigin() instanceof QueryOperationCatalogView) {
        ...
        extractFormattedComment(view)
            .ifPresent(
                (c) -> {
                  stringBuilder.append(String.format(" COMMENT '%s'%s", c, System.lineSeparator()));
                });
